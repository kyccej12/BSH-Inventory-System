<?php
	...
	class authenticate extends myDB {
		...
		function verify($uname,$pass,$payclass) {
			if(!empty($uname) && !empty($pass)) {
				...
				if(mysqli_num_rows($res) > 0) {	
					...
					if(!empty($uid)) {
						$this->storeSession($uid,$utype,$payclass);
						return true;
					} else {
						return false;
					}
		...
		function storeSession($uid,$utype,$payclass) {
	...
	if($auth->verify($_POST['txtname'],$_POST['txtpass'],$_POST['payclass']) == true) {
