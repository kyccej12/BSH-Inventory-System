<?php
	...
	if(isset($_GET['id']) && $_GET['id'] != "") { 
		...
		list($end) = $con->getArray("select sum(purchases+inbound-outbound-pullouts-sold) as currentbalance from ibook where item_code = '$res[item_code]' and doc_date between '2023-05-22' and '".date('Y-m-d')."';");
		

		list($onPO) = $con->getArray("select sum(b.qty) from po_header a left join po_details b on a.po_no = b.po_no where b.item_code = '$res[item_code]' and a.status = 'Finalized' and b.qty_dld = 0;");
	}
...
<form name="merchandise" id="merchandise">
	...
	<table width="100%" cellspacing="0" cellpadding="5" style="border-bottom: 1px solid black; margin-bottom: 5px;">
		<tr>
			<td align=left>
				<a href="#" onClick="savePInfo(<?php echo $res['record_id']; ?>);" class="topClickers"><img src="images/icons/floppy.png" width=18 height=18 align=absmiddle />&nbsp;Add/Save Changes Made</a>
				<?php if($res['record_id'] != "") { ?>
				&nbsp;&nbsp;<a href="#" onClick="deletePro('<?php echo $_GET['id']; ?>');" class="topClickers"><img src="images/icons/delete.png" width=18 height=18 align=absmiddle />&nbsp;Delete Record</a>
				<?php } ?>
			</td>
			<td align=right>
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0" >
		<tr>
			...
			<td width="40%">
				<select name="item_category" id="item_category" style="width: 70%;" class="nInput" <?php if($res['record_id'] == '') { echo "onChange=\"javascript: getMyCode(); \""; } ?>>
		...
		<tr>
			...
			<td width="10%" class="spandix-l">Barcode :</td>
			<td width="40%">
				<input type="text" name="item_barcode" id="item_barcode" style="width: 70%;" class="nInput" value="<?php echo $res['barcode']; ?>">
			</td>
		</tr>
		<tr><td height=1></td></tr>
		<tr>
		...
		<tr>
			<td style="padding: 0px 0px 1px 0px;">
				<div id="custmenu" align=left class="ddcolortabs">
					<ul class=float2>
						<li><a href="#" <?php getMod("1",$_GET[mod]); ?> onclick="javascript: changeMod(1);"><span id="tbbalance3">General</span></a></li>
					</ul>
				</div>
			</td>
		</tr>
	...
	<table width="100%" cellpadding=0 cellspacing=1 class="td_content" style="padding:10px;" border=0>
		<tr>
			...
			<td width=85% colspan=3><textarea name="item_fdescription" id="item_fdescription" style="width: 100%;" rows=2><?php echo $res['full_description']; ?></textarea></td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td width=35% align=right>
				<select name="asset_acct" id="asset_acct" style="width: 100%;" class="nInput">
					<option value="">- NA -</option>
					<?php
						$iun = $con->dbquery("select acct_code,description from acctg_accounts where acct_grp in ('3') and description not like ('%accu%') order by description;");
		...
		<tr>
			...
			<td width=55% class="spandix-l" colspan=2 align=right><b>Qty Available : &nbsp;&nbsp;</b><?php echo number_format($end,2); ?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>Qty-On-PO : &nbsp;&nbsp;</b><?php echo number_format($onPO,2);?></td>
		</tr>
		<tr>
			<td width=15% class="spandix-l"></td>
			<td width=30%>
		...
		<tr>
			...
			<td width=55% colspan=2 align=right class="spandix-l">
				Preferred Supplier : <input type="text" class="inputSearch2" style="width: 70px; padding-left: 22px;margin-bottom:2px;" id="supplier" name="supplier" value="<?php echo $res['supplier']; ?>"> <input type="text" name="supplier_name" id="supplier_name" style="width: 55%;" class="nInput" value="<?php echo $sname; ?>">
			</td>
			<td width=20% class="spandix-l"></td>
			<td width=35%></td>
		</tr>
...
<form name="changeModPage" id="changeModPage" action="items.details.php" method="GET" >
	<input type="hidden" name="icode" id="icode" value="<?php echo $res['item_code']; ?>">
